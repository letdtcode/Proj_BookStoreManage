...
create or alter proc sp_DeleteCategory
@idCategory  int
as
begin
	delete from BookCategory where idCategory = @idCategory
	delete from Category  where idCategory = @idCategory
end
-------------Procedure sửa Category
create proc sp_UpdateCategory
@idCategory  int, @nameCategory  nvarchar(20)
as
begin
	update  Category  set nameCategory = @nameCategory where idCategory = @idCategory
end
--------------Procedure tìm kiếm Category gần đúng theo tên 
create proc sp_SearchCategoryByName
@nameCategory  nvarchar(20)
as
begin
	select *
	from CATEGORY
	where dbo.fuConvertToUnsign1(nameCategory) like N'%' + dbo.fuConvertToUnsign1(@nameCategory)+ '%' 
end
--------------Procedure thêm Book_Category
Create proc sp_InsertBookCategory
@idBook int, @idCategory int
as
begin	
	insert into BOOK_CATEGORY (idBook,idCategory) values(@idBook,@idCategory)
end
exec sp_InsertBookCategory @idBook=1, @idCategory =2
--------------Procedure xóa Book_Category
Create proc sp_DeleteBookCategory
@idBook int, @idCategory int
as
begin	
	delete from BOOK_CATEGORY where idBook=@idBook and idCategory= @idCategory
end
---------------Procedure sửa Book_Category theo idBook
Create proc sp_UpdateBookCategoryByIdBook
@idBook int, @idCategory int
as
begin	
	update  BOOK_CATEGORY set idCategory= @idCategory where idBook=@idBook
end
=======
>>>>>>> 6c4dbd0f869addc8f62a7c61b9f98b1a4b781430
