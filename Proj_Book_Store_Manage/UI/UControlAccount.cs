...
namespace Proj_Book_Store_Manage.UI
{
    public partial class UControlAccount : UserControl
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (utl.CheckIDValid is true)
                {
                    ...
                    {
                       // account.deleteAccount(utl.row, ref err);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (isAdd)
                {
                   ...
                   // account.addNewAccount(this.txtUserName.Text, this.txtPassword.Text, int.Parse(this.cbTypeAcc.SelectedItem.ToString()), int.Parse(this.cbEmployee.SelectedItem.ToString()), ref err);
                    if (err is null)
                    {
                ...
                {
                    //account = new AccountBL()
                    //account.modifyAccount(utl.row, this.txtUserName.Text, this.txtPassword.Text, int.Parse(this.cbTypeAcc.SelectedItem.ToString()), int.Parse(this.cbEmployee.SelectedItem.ToString()), ref err);
                    //LoadData();
        ...
        private void dgvAuthor_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            cbEmployee.ValueMember = dgvAuthor.Rows[utl.rowCurrent].Cells[4].Value.ToString();
        }
    }
}
